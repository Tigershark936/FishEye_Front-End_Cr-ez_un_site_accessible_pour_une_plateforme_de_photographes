export function photographerTemplate(data, index) {
  ...
  function getUserCardDOM() {
    ...
    // Container de l’image du photographe
    ...
    h2Name.setAttribute("id", nameId); // ID pour aria-labelledby du lien parent
    h2Name.textContent = name;

    const h3Location = document.createElement("h3");
    h3Location.textContent = `${city}, ${country}`;
    h3Location.setAttribute("aria-label", `Localisation : ${city}, ${country}`);

    const h4Tagline = document.createElement("h4");
    h4Tagline.textContent = `${tagline}`;
    h4Tagline.setAttribute("aria-label", `Phrase d'accroche : ${tagline}`);

    const pPrice = document.createElement("p");
    pPrice.textContent = `${price}€/jour`;
    pPrice.setAttribute("aria-label", `Tarif : ${price} euros par jour`);
